...
class Payment {
	...
	async readCard(isContacless) {
		const {
			cardNumber,
			expirationMonth,
			expirationYear
		} = await MT188CardReaderInstance.getPANWithRetries({ maxAttempts: 10, contactless: isContacless });
	 ...
	 */
	async collectWithCart(cart, paymentSystem, isCash, isContacless) {
		const paymentData = {
			cart,
			slaId: this.slaId,
		...
		const fields = isCash ? null : await this.readCard(isContacless);
	 ...
	 * @param {boolean} isCash
	 */
	async collectWithTransactionId(value, paymentSystem, transactionId, isCash, isContacless) {
		const OrderInstance = new Order({
			environment: this.environment,
			apiKey: this.apiKey,
			apiToken: this.apiToken
	 ...
	 * @param {boolean} isCash
	 */
	collect(value, paymentSystem, transactionId, cart, isCash = false, isContacless = false) {
		if(!value)
		...
		if(transactionId)
			return this.collectWithTransactionId(value, paymentSystem, transactionId, isCash, isContacless);

		return this.collectWithCart(cart, paymentSystem, isCash, isContacless);
	}
}
